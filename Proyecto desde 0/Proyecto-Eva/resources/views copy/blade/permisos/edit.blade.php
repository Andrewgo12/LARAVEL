			...
			<div class="box-body">
				<div class="container">
					<div class="table-responsive">
						<form action="{{ url('administrador/Cpermisos/update') }}" method="POST" class="form-group">
							@csrf
							@method('POST')
							<div class="row">
								<div class="form-group">
									...
									<select disabled="disabled" name="menu_id" id="menu" required="required" style="width: 70%" class="form-control col-sm-7">
										...
										@foreach($menus as $menu)
											<option value="{{ $menu->id }}" {{ $menu->id == $permiso->menu_id ? 'selected' : '' }}>{{ $menu->nombre }}</option>
										@endforeach
									</select>
								</div>
							</div>
								...
								<div class="form-group">
									...
									<select disabled="disabled" name="rol_id" id="rol" required="required" style="width: 70%" class="form-control col-sm-7">
										...
										@foreach($roles as $rol)
											<option  value="{{ $rol->id }}" {{ $rol->id == $permiso->rol_id ? 'selected' : '' }}>{{ $rol->nombre }}</option>
										@endforeach
									</select>
								</div>
							</div>
