  ...
  <table cellpadding=0 cellspacing=0 width=832 class="tabla_detalles" style='border-collapse:
    ...
    <tr height=24 style='height:18.0pt'>
      ...
      <td colspan=17 rowspan=12 class=xl9626419 width=309 style='border-right:1.0pt solid black;
						border-bottom:1.0pt solid black;width:234pt'>&nbsp;<img class="" width="350px" height="280px" src="{{ asset('') }}assets/upload_imagenes/{{ $equipo->image }}" alt=""></td>
      <td class=xl1526419></td>
    </tr>
    ...
    <tr height=21 style='height:15.75pt'>
      ...
      <td colspan=35 class=xl13626419 width=361 style='border-right:1.0pt solid black;
        ...
        @if($equipo->registro_sanitario != null)
          ...
          <a target="__blank" class="glyphicon glyphicon-file esconder" href="{{ asset('') }}assets/upload_registros_sanitarios/{{ $equipo->file_registro_sanitario }}"></a>
        @else
          {{ $equipo->invima }}
      ...
      <td colspan=28 class=xl11726419 width=272 style='border-right:1.0pt solid black;
        ...
        @if($equipo->orden_compra_id != "" && $equipo->orden_compra_id != null)
          ...
          @if($equipo->file_orden_compra != null && $equipo->file_orden_compra != "")
            <a class="glyphicon glyphicon-file esconder" href="{{ asset('') }}assets/upload_ordenes_compra/{{ $equipo->file_orden_compra }}" target="__blank"></a>
          <?php endif ?>
        <?php endif ?>
        ...
        <?php if (isset($archivo)) : ?>
          <ul>
            @foreach($archivo as $archivo_row)
              <li>Archivo: {{ $archivo_row->file }} <a style="color: #222d32;font-size:15px;font-weight:900;" href="{{ asset('') }}assets/upload_archivos/{{ $archivo_row->file }}" onclick="window.open(this.href, 'mywin',
    ...
    <tr height=21 style='height:15.75pt'>
      <td colspan=54 height=21 class=xl12726419 width=752 style='border-right:1.0pt solid black;
        ...
        <div class="row row-owner">
          ...
          <div class="col-sm-4">
            <img class="img-responsive" src="{{ asset('') }}assets/upload_imagenes/{{ $equipo->propietario_logo }}" alt="Logo del propietario">
          </div>
          @if($equipo->baja_id != 0)
            <div class="col-sm-4">
              <h5><u>Este equipo ha sido relacionado a inventaio para su correspondiente disposición final</u></h4>
                ...
                {{ $equipo->baja }}
                <a class="fa fa-paperclip" target="__blank" href="{{ asset('') }}assets/upload_bajas/{{ $equipo->baja_documento }}"></a>
            </div>
          <?php endif ?>
        </div>
      </td>
        ...
        <td colspan=54 height=22 class=xl12726419 width=752 style='border-right:1.0pt solid black;
          ...
          <ul>
            @foreach($archivos as $archivo_singular)
              <li>
                {{ $archivo_singular->archivo }}({{ $archivo_singular->otro }})<a target="__blank" class="glyphicon glyphicon-file esconder" href="<?= base_url(); ?>assets/upload_equipo_archivos/{{ $archivo_singular->vinculo }}"></a>
              </li>
            <?php endforeach ?>
          </ul>
        </td>
              ...
              @else
                <p> <strong>{{ $observacion_row->created_at }}</strong>
                  @if($observacion_row->file != null && $observacion_row->file != "")
                    <a href="<?= base_url(); ?>assets/upload_observaciones/{{ $observacion_row->file }}" target="__blank" class="glyphicon glyphicon-file"></a>
                  <?php endif ?>
                  <br>
