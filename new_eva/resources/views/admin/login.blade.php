<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <title>AdminLTE 2 | Log in</title>
  <!-- Tell the browser to be responsive to screen width -->
  <meta content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no" name="viewport">
  
  <!-- url('/') = http://localhost/ventas_ci/-->

  <!-- Bootstrap 3.3.7 -->
  <link rel="stylesheet" href="{{ url('/') }}assets/template/bootstrap/css/bootstrap.min.css">
  <!-- Font Awesome -->
  <link rel="stylesheet" href="{{ url('/') }}assets/template/font-awesome/css/font-awesome.min.css">

  <!-- Theme style -->
  <link rel="stylesheet" href="{{ url('/') }}assets/template/dist/css/AdminLTE.min.css">

</head>
<body class="hold-transition login-page">
  <div class="login-box">
    <div class="login-logo">
      <h2>SISTEMA DE VENTAS</h2>
    </div>
    <!-- /.login-logo -->
    <div class="login-box-body">
      <p class="login-box-msg">Introduzca sus datos de ingreso</p>

      @if($this->session->flashdata('error'))
        <div class="alert alert-danger">
          <p>{{ $this->session->flashdata('error') }}</p>
        </div>

      <?php endif ?>

      <form action="{{ url('/'); }}Cauth/login" method="post">
        <div class="form-group has-feedback">
          <input autofocus="" type="text" class="form-control" placeholder="Usuario" name="username">
          <span class="glyphicon glyphicon-user form-control-feedback"></span>
        </div>
        <div class="form-group has-feedback">
          <input type="password" class="form-control" placeholder="Password" name="password">
          <span class="glyphicon glyphicon-lock form-control-feedback"></span>
        </div>
        <div class="row">
          <div class="col-xs-12">
            <button type="submit" class="btn btn-primary btn-block btn-flat">Ingresar</button>
          </div>
          <!-- /.col -->
        </div>
      </form>
      <br>
      <button data-toggle="modal" data-target="#modal_reg_usuario" type="submit" class="btn btn-success btn-block btn-flat">Registrarse</button>


    </div>
    <!-- /.login-box-body -->
  </div>
  <!-- /.login-box -->

  <!-- jQuery 3 -->
  @push('scripts')
<script>

</script>
@endpush
  <!-- Bootstrap 3.3.7 -->
  @push('scripts')
<script>

</script>
@endpush
  <!-- javascript Modal -->
  @push('scripts')
<script>

</script>
@endpush

</body>
</html>
