                    ...
                    @foreach ($guias as $guia)
                      @if ($guia->id != 0)
                        <tr>
                          ...
                          <td>
                            {{ $guia->name }}
                            <a class="glyphicon glyphicon-paperclip updateCounter" data-id="{{ $guia->id }}" target="_blank" href="{{ asset('assets/upload_guias/' . $guia->file) }}">
                            </a>
                          </td>
                          <td>
                            {{ $guia->totalQuery }}
...
<script>
  ...
  var insertar_guia = "{{ session('acciones.21.insertar', '0') }}";
  var editar_guia = "{{ session('acciones.21.editar', '0') }}";
  var eliminar_guia1 = "{{ session('acciones.21.eliminar', '0') }}";
