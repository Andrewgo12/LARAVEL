            ...
            <div class="panel panel-default">
              <div class="panel-body">
                <ul class="list-inline section-filter-navbar">
                  ...
                  <li class="list-inline-item custom-btn"><a class="btn btn-default" href="{{ url('equipo/Cequipos/Exportar_cantidades') }}" target="_blank">Month</a>
                  </li>
                </ul>
              </div>
            </div>
...
<script>
  ...
  var sede_id = "{{ session('sede_id', '') }}";

  var insertar_equipo = "{{ session('acciones.0.insertar', 0) }}"; //equipos
  var editar_equipo = "{{ session('acciones.0.editar', 0) }}"; //equipos

  var insertar_equipo_archivo = "{{ session('acciones.13.insertar', 0) }}"; //equipo archivos
  var leer_equipo_archivo = "{{ session('acciones.13.leer', 0) }}"; //equipo archivos

  var insertar_baja = "{{ session('acciones.4.insertar', 0) }}"; //bajas

  var insertar_observacion = "{{ session('acciones.17.insertar', 0) }}"; //observaciones
  var eliminar_observacion = "{{ session('acciones.17.eliminar', 0) }}"; //observaciones
  var editar_observacion = "{{ session('acciones.17.editar', 0) }}"; //observaciones

  var insertar_servicio = "{{ session('acciones.2.insertar', 0) }}"; //servicios

  var insertar_area = "{{ session('acciones.19.insertar', 0) }}"; //areas
  var editar_area = "{{ session('acciones.19.editar', 0) }}"; //areas
  var eliminar_area = "{{ session('acciones.19.eliminar', 0) }}"; //areas

  var insertar_contingencia = "{{ session('acciones.20.insertar', 0) }}"; //contingencias
  var eliminar_contingencia = "{{ session('acciones.20.eliminar', 0) }}"; //contingencias
  var editar_contingencia = "{{ session('acciones.20.editar', 0) }}"; //contingencias

  var insertar_soporte_compra = "{{ session('acciones.6.insertar', 0) }}"; //soportes compra
