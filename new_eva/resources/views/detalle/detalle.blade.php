		<div class="">
			...
			<div class="table-responsive" id="contenedor_detalle_equipo">
				...
				<div class="tg-wrap"><table class="tg">
					<tr><th class="text-center"><div class="logo-hospital"> <img src="{{ url('/'); }}assets/template/logo_hospital.jpg" width="100px;" height="100px;"></div></th>
					...
					<tr>
						...
						<td class="tg-ng7p" colspan="5" rowspan="9">
							<img  class="" width="350px" height="280px" src="{{ url('/') }}assets/upload_imagenes/{{ $equipo->image }}" alt="">
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td class="tg-0akb" colspan="5">{{ $equipo->garantia }}</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td class="tg-0pky" colspan="9">
							@if(!empty($contactos))
								<div class="col-sm-12 col-md-12 col-lg-12 @if(empty($contactos))
									esconder
								<?php endif ?>">
			...
			<tr>
				...
				<td class="tg-0pky" colspan="9" rowspan="3">
					@if(!empty($especificaciones))
						<div class="row">
							<div class="col-sm-12 col-md-12 col-lg-12 @if(empty($especificaciones))
								esconder
							<?php endif ?>">
		...
		<tr class="esconder">
			...
			<td class="tg-0pky">{{ ($equipo->fecha_mantenimiento=="0000-00-00"||$equipo->fecha_mantenimiento==""||$equipo->fecha_mantenimiento==null)?"":$equipo->fecha_mantenimiento }}</td></td>

			<td class="tg-0akb">Estado actual del equipo:</td>
			<td class="tg-0pky" colspan="5">{{ $equipo->estadoequipos }}
			</tr>
			<tr>
					...
					<table class="table table-bordered tblCorrectivos" >
						...
						<tbody>
							@foreach($correctivos_original as $correctivo)
								<tr>
									<td>{{ $correctivo->id }}</td>
									<td>{{ $correctivo->descripcion }}</td>
									<td>{{ $correctivo->estado }}</td>
									<td>
										@if($correctivo->image!=""&&$correctivo->image!=null)

											<a  target='_blank' href='{{ url('/') }}assets/upload_correctivos/{{ $correctivo->image; }}' class='btn bnt-info'><span class='glyphicon glyphicon-file'></span><br>

											<?php endif ?>
											@if($correctivo->file_diagnostico!=""&&$correctivo->file_diagnostico!=null)

												<a  target='_blank' href='{{ url('/') }}assets/upload_correctivos/{{ $correctivo->file_diagnostico; }}' class='btn bnt-info'><span class='glyphicon glyphicon-file'></span><br>
												<?php endif ?>

												@if($correctivo->file_cierre!=""&&$correctivo->file_cierre!=null)

													<a  target='_blank' href='{{ url('/') }}assets/upload_correctivos/{{ $correctivo->file_cierre; }}' class='btn bnt-info'><span class='glyphicon glyphicon-file'></span><br>
								...
								<table class="table table-bordered tblCorrectivosGenerales" >
									...
									<tbody>
										@foreach($correctivos_generales_original as $correctivo_general)
											<tr>
												<td>{{ $correctivo_general->code }}</td>
												<td>{{ $correctivo_general->description }}</td>
												<td>{{ $correctivo_general->fecha_mantenimiento }}</td>
												@if($correctivo_general->file!=""&&$correctivo_general->file!=null)

													<td><a  target='_blank' href='{{ url('/') }}assets/upload_correctivos_generales/{{ $correctivo_general->file; }}' class='btn bnt-info'><span class='glyphicon glyphicon-file'></span></td>
												<?php endif ?>
											</tr>
								...
								<table class="table table-bordered tblPreventivos" >
									...
									<tbody>
										@foreach($preventivos_original as $preventivo)
											<tr>
												<td>{{ $preventivo->description }}</td>
												<td>{{ $preventivo->fecha_mantenimiento }}</td>
												<td>{{ $preventivo->fecha_programada }}</td>
												@if($preventivo->file!=""&&$preventivo->file!=null)

													<td><a  target='_blank' href='{{ url('/') }}assets/upload_preventivos/{{ $preventivo->file; }}' class='btn bnt-info'><span class='glyphicon glyphicon-file'></span></td>
												<?php endif ?>
											</tr>
								...
								<table class="table table-bordered tblCalibraciones" >
									...
									<tbody>
										@foreach($calibraciones_original as $calibracion)
											<tr>
												<td>{{ $calibracion->description }}</td>
												<td>{{ $calibracion->fecha_calibracion }}</td>
												<td>{{ $calibracion->fecha_programada }}</td>
												@if($calibracion->file!=""&&$calibracion->file!=null)

													<td><a  target='_blank' href='{{ url('/') }}assets/upload_calibraciones/{{ $calibracion->file; }}' class='btn bnt-info'><span class='glyphicon glyphicon-file'></span></td>
												<?php endif ?>
											</tr>
								...
								<div class="text-center">
									<ul>
										@foreach($archivos as $archivo_singular)
											<li >
												{{ $archivo_singular->archivo }}<a target="__blank" class="glyphicon glyphicon-file esconder" href="{{ url('/'); }}assets/upload_equipo_archivos/{{ $archivo_singular->vinculo }}"></a>
											</li>
										<?php endforeach ?>				
									</ul>
								</div>
