...
/**
 * Controlador Cobservaciones - Sistema HUV
 * Gestiona las funcionalidades del módulo correspondiente
 */
class Cobservaciones extends Controller
{
    /**
...
/* Sistema HUV */
    public function index()
    {
        ...
        return view('laravel.observaciones.list', $data);
    }
    ...
    public function getServerSide(Request $request): JsonResponse
    {
        try {
            $params = $request->all();
            $result = Mobservaciones::getServerSide($params);
    ...
    public function add(Request $request): JsonResponse
    {
        try {
            ...
            $result = Mobservaciones::add($data);
    ...
    public function update(Request $request): JsonResponse
    {
        try {
            ...
            $result = Mobservaciones::edit($data);
    ...
    public function delete(Request $request): JsonResponse
    {
        try {
            $id = $request->input('id');
            $result = Mobservaciones::remove($id);
    ...
    public function getByEquipo(Request $request): JsonResponse
    {
        try {
            $data = $request->all();
            $result = Mobservaciones::getByEquipo($data);

            return response()->json($result, 200);
        } catch (\Exception $e) {
            return response()->json(['error' => 'Error en getByEquipo: ' . $e->getMessage()], 500);
        }
    }
    ...
    public function getByTipo(Request $request): JsonResponse
    {
        try {
            $data = $request->all();
            $result = Mobservaciones::getByTipo($data);
    ...
    public function getTipos(Request $request): JsonResponse
    {
        try {
            $data = $request->all();
            $result = Mobservaciones::getTipos($data);
    ...
    public function getHistorial(Request $request): JsonResponse
    {
        try {
            $data = $request->all();
            $result = Mobservaciones::getHistorial($data);
