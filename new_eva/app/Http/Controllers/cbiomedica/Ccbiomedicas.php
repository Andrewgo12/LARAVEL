...
/**
 * Controlador Ccbiomedicas - Sistema HUV
 * Gestiona las funcionalidades del módulo correspondiente
 */
class Ccbiomedicas extends Controller
{
    /**
    ...
    /**
namespace App\Http\Controllers\Cbiomedica;
...
/* Sistema HUV */
    public function index()
    {
        ...
        return view('laravel.cbiomedicas.list', $data);
    }
    ...
    public function getServerSide(Request $request): JsonResponse
    {
        try {
            $params = $request->all();
            $result = Mcbiomedicas::getServerSide($params);
    ...
    public function getAll(Request $request): JsonResponse
    {
        try {
            $data = $request->all();
            $result = Mcbiomedicas::getAll($data);
    ...
    public function getClasificaciones(Request $request): JsonResponse
    {
        try {
            $data = $request->all();
            $result = Mcbiomedicas::getClasificaciones($data);

            return response()->json($result, 200);
        } catch (\Exception $e) {
            return response()->json(['error' => 'Error en getClasificaciones: ' . $e->getMessage()], 500);
        }
    }

    /* Sistema HUV */
    public function addClasificacion(Request $request): JsonResponse
    {
        try {
            $data = $request->all();
            $result = Mcbiomedicas::addClasificacion($data);

            return response()->json($result, 200);
        } catch (\Exception $e) {
            return response()->json(['error' => 'Error en addClasificacion: ' . $e->getMessage()], 500);
        }
    }

    /* Sistema HUV */
    public function updateClasificacion(Request $request): JsonResponse
    {
        try {
            $data = $request->all();
            $result = Mcbiomedicas::updateClasificacion($data);

            return response()->json($result, 200);
        } catch (\Exception $e) {
            return response()->json(['error' => 'Error en updateClasificacion: ' . $e->getMessage()], 500);
        }
    }
